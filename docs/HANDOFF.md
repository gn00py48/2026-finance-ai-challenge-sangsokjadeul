...
추가로 C3(내 로드맵)를 다시 봤다. 처음 매핑에서 "구현"으로 적었으나 실제로는 Figma와 구성이 달랐다.
Figma C3는 가로 캐러셀 카드, "단계 상태" 범례 카드, 안내, 세로 목록 네 블록으로 이뤄지는데 현재 화면은
제목과 단순 버튼 목록뿐이었다. 전용 화면으로 다시 만들었다.

| 항목 | Figma | 수정 전 | 조치 |
| --- | --- | --- | --- |
| 화면 구성 | 캐러셀 카드 + 범례 카드 + 안내 + 세로 목록 | `Page` 제목 + 버튼 목록 | 네 블록으로 재작성 |
| 헤더 우측 | `3 / 7` 진행 표기 | 없음 | 로드맵 화면에만 현재/전체 단계 수 표기 |
| 목록 행 부제 | 완료일 / `D-42 · 처리 중` / `기산일 확정 후 계산` | 기한 문자열만 | 상태별로 나눠 표기 |
| 목록 배지 | 현재도 회색(`Neutral/050`) | 진한 파랑 | 로드맵 목록에서만 회색으로 |
| 완료 배지 색 | `Success/Background` #EDF5F0, `Success/Foreground` #3F7160 | #edf5f2 / #397667 | 토큰 값으로 교체 |
| 캐러셀 원 지름 | 28px, 현재 38px | 24px, 현재 32px | 값 교체 |
| 카드 간격 | 16px | 12px | 로드맵 화면만 16px |
...
3. **매핑표의 "구현" 표시를 다시 검증한다.** 아래 "대조 방법의 한계"를 먼저 읽는다.
4. B1의 "고인의 표시명" 입력을 명세와 대조해 유지 여부를 정한다.
5. B7(업로드·분석 진행)은 API-6이 정해진 뒤에 만든다.

### 대조 방법의 한계 — C3에서 드러난 것

1차 대조는 Figma 노드 트리(레이어 이름과 좌표)를 기준으로 했다. 그런데 화면 내용이 컴포넌트 인스턴스
(`Card/Content`, `Card/Roadmap`, `Card/Asset` 등)로 들어가 있으면 트리에 자식이 보이지 않는다. 이 경우
**"카드가 세 개 있다" 수준까지만 알 수 있고 그 안에 무엇이 들어가는지는 알 수 없다.**

C3(내 로드맵)가 여기 걸렸다. 트리에는 `Card/Content` + `Card/Roadmap` + `Feedback/Notice` + `Card/Asset`으로만
보여 기존 목록 화면과 맞는다고 판단해 "구현"으로 적었다. 실제로는 가로 캐러셀, 단계 상태 범례, 안내,
세로 목록 네 블록이었고 화면에는 제목과 버튼 목록뿐이었다. 뒤늦게 `get_design_context`로 인스턴스 내부를
펼쳐 보고서야 확인했다.

따라서 **인스턴스 내부를 확인하지 않은 채 "구현"으로 적은 화면은 아직 검증되지 않은 것으로 본다.**
렌더 이미지나 `get_design_context`로 내부를 확인한 화면은 C1·D2·D3·D5·B10·E1·H2·H1·C3뿐이다.
나머지(C2·C4·F1·G1·G3·H4·D1·D4·B12 등)는 같은 방식으로 다시 확인해야 한다.

이 작업에서 얻은 부수 정보가 하나 있다. C3의 인스턴스를 펼치면서 Figma 7단계 중 5개의 이름이 확인됐다.
`1 사망신고`, `2 안심상속 원스톱 조회`, `3 채무 확인`, `4 상속 방식 판단`, `5 취득세 신고`이며 6·7단계는
Figma에도 나열되어 있지 않다. API-1을 결정할 때 쓸 수 있다.
