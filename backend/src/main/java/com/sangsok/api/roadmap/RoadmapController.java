...
public class RoadmapController {
    private final RoadmapService service;private final CaseService access;private final RoadmapStepRepository steps;private final TaskResultRepository results;private final WarningRepository warnings;private final FinancialItemRepository items;private final DocumentRepository documents;
    public RoadmapController(RoadmapService service,CaseService access,RoadmapStepRepository steps,TaskResultRepository results,WarningRepository warnings,FinancialItemRepository items,DocumentRepository documents){this.service=service;this.access=access;this.steps=steps;this.results=results;this.warnings=warnings;this.items=items;this.documents=documents;}
    @PostMapping({"/api/cases/{caseId}/roadmaps","/api/cases/{caseId}/roadmaps/recalculate"}) Object generate(@PathVariable Long caseId,Authentication auth){return roadmap(service.generate(access.owned(caseId,auth)));}
    ...
    @PatchMapping("/api/tasks/{id}/result") Object result(@PathVariable Long id,@RequestBody ResultInput in,Authentication auth){RoadmapStep s=ownedStep(id,auth);return step(service.updateResult(s,in.progressStatus(),in.resultDate(),in.resultText(),in.memo()));}
    ...
    private Object roadmap(Roadmap m){return Map.of("id",m.getId(),"version",m.getVersion(),"rulesVersion",m.getRulesVersion(),"createdAt",m.getCreatedAt(),"steps",steps.findAllByRoadmapIdOrderBySequenceNo(m.getId()).stream().map(this::step).toList());}
    ...
    private Object warning(Warning w){return Map.of("id",w.getId(),"category",w.getCategory(),"level",w.getLevel(),"title",w.getTitle(),"message",w.getMessage(),"navigationTarget",w.getNavigationTarget(),"resolved",w.isResolved());}
