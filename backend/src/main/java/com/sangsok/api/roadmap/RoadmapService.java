...
public class RoadmapService {
    ...
    private void rebuildWarnings(InheritanceCase c,Roadmap m){warnings.deleteAllByInheritanceCaseId(c.getId());List<FinancialItem> inv=items.findAllByInheritanceCaseIdOrderByCreatedAtDesc(c.getId());if(inv.stream().anyMatch(x->x.getAmountStatus()==AmountStatus.NEEDS_CONFIRMATION))warn(c,null,WarningCategory.MISSING_INFO,WarningLevel.HIGH,"금액 확인 필요","금액을 확인하지 못한 재산·채무가 있습니다.",null,NavigationTarget.FINANCIAL_ITEM_LIST);if(!c.isAwarenessDateCertain()||c.getAwarenessDate()==null)warn(c,null,WarningCategory.DEADLINE_RISK,WarningLevel.CRITICAL,"기산일 확인 필요","상속개시 사실을 안 날이 불확실하여 관련 기한을 확정하지 않았습니다.",null,NavigationTarget.CASE_EDIT);if(inv.stream().anyMatch(x->x.getAssetOrDebt()==AssetOrDebt.DEBT))warn(c,null,WarningCategory.CHECK_NOW,WarningLevel.HIGH,"채무 정보 확인","등록된 채무가 있습니다. 단정적인 선택 전에 공식 안내와 전문가 확인을 권합니다.",null,NavigationTarget.FINANCIAL_ITEM_LIST);}
