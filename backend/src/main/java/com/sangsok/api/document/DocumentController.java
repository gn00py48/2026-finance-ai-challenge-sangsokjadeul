...
public class DocumentController {
    ...
    private final DocumentRepository documents;private final DocumentAnalysisRepository analyses;private final FinancialItemRepository items;private final CaseService access;private final AiDocumentAnalyzer analyzer;private final ObjectMapper mapper;private final DocumentStorage storage;private final PiiTextMasker masker;
    public DocumentController(DocumentRepository documents,DocumentAnalysisRepository analyses,FinancialItemRepository items,CaseService access,AiDocumentAnalyzer analyzer,ObjectMapper mapper,DocumentStorage storage,PiiTextMasker masker){this.documents=documents;this.analyses=analyses;this.items=items;this.access=access;this.analyzer=analyzer;this.mapper=mapper;this.storage=storage;this.masker=masker;}
    ...
    @PostMapping("/api/documents/{id}/analyze") @Transactional Object analyze(@PathVariable Long id,Authentication auth){Document d=owned(id,auth);d.setStatus(DocumentStatus.ANALYZING);documents.save(d);DocumentAnalysis a=new DocumentAnalysis();a.setDocument(d);a.setStatus("ANALYZING");try{AnalysisResult result=maskResult(analyzer.analyze(storage.get(d.getStorageKey()),d.getMimeType()));a.setDocumentType(result.documentType());a.setResultJson(mapper.writeValueAsString(result));a.setStatus("NEEDS_REVIEW");d.setStatus(DocumentStatus.NEEDS_REVIEW);}catch(Exception e){a.setStatus("FAILED");a.setErrorMessage("분석 결과를 처리하지 못했습니다.");d.setStatus(DocumentStatus.FAILED);}analyses.save(a);documents.save(d);return view(d);}
    @PatchMapping("/api/documents/{id}/confirm") @Transactional Object confirm(@PathVariable Long id,@Valid @RequestBody AnalysisResult requested,Authentication auth){Document d=owned(id,auth);if(d.getStatus()!=DocumentStatus.NEEDS_REVIEW)throw new ApiException(HttpStatus.CONFLICT,"ANALYSIS_NOT_REVIEWABLE","검토 가능한 분석 결과가 아닙니다.");if(items.existsBySourceDocumentId(id))throw new ApiException(HttpStatus.CONFLICT,"ALREADY_CONFIRMED","이미 확정된 문서입니다.");AnalysisResult result=maskResult(requested);for(AnalysisResult.Item i:result.items()){FinancialItem x=new FinancialItem();x.setInheritanceCase(d.getInheritanceCase());x.setSourceDocument(d);x.setInstitution(i.institution());x.setItemType(i.category());x.setAssetOrDebt(i.assetOrDebt());if(i.amountStatus()==AmountStatus.CONFIRMED&&i.amount()==null)throw new ApiException(HttpStatus.BAD_REQUEST,"AMOUNT_REQUIRED","확정 금액을 입력해 주세요.");x.setAmountStatus(i.amountStatus());x.setAmount(x.getAmountStatus()==AmountStatus.CONFIRMED?i.amount():null);x.setReferenceDate(i.referenceDate());x.setConfidence(i.confidence());x.setEvidenceText(i.evidenceText());items.save(x);}analyses.findFirstByDocumentIdOrderByCreatedAtDesc(id).ifPresent(a->{try{a.setResultJson(mapper.writeValueAsString(result));a.setStatus("CONFIRMED");analyses.save(a);}catch(com.fasterxml.jackson.core.JsonProcessingException e){throw new IllegalStateException("검수 결과를 저장하지 못했습니다.",e);}});d.setStatus(DocumentStatus.CONFIRMED);d.getInheritanceCase().setRoadmapDirty(true);documents.save(d);return view(d);}
    ...
    private AnalysisResult maskResult(AnalysisResult result){return new AnalysisResult(masker.mask(result.documentType()),result.items().stream().map(i->new AnalysisResult.Item(masker.mask(i.institution()),i.category(),i.assetOrDebt(),i.amount(),i.amountStatus(),i.referenceDate(),i.confidence(),masker.mask(i.evidenceText()))).toList(),result.warnings().stream().map(masker::mask).toList());}
