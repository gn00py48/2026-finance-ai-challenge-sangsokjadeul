...
public class DocumentController {
    ...
    private final DocumentRepository documents;private final DocumentAnalysisRepository analyses;private final FinancialItemRepository items;private final CaseService access;private final AiDocumentAnalyzer analyzer;private final ObjectMapper mapper;private final Path root;
    public DocumentController(DocumentRepository documents,DocumentAnalysisRepository analyses,FinancialItemRepository items,CaseService access,AiDocumentAnalyzer analyzer,ObjectMapper mapper,@Value("${app.storage.path}") String root){this.documents=documents;this.analyses=analyses;this.items=items;this.access=access;this.analyzer=analyzer;this.mapper=mapper;this.root=Paths.get(root).toAbsolutePath().normalize();}
    ...
    Object upload(@PathVariable Long caseId,@RequestPart MultipartFile file,@RequestParam(defaultValue="false") boolean aiConsent,Authentication auth)throws IOException{InheritanceCase c=access.owned(caseId,auth);if(!aiConsent)throw new ApiException(HttpStatus.BAD_REQUEST,"AI_CONSENT_REQUIRED","AI 분석 동의가 필요합니다.");validate(file);Files.createDirectories(root);String key=UUID.randomUUID()+extension(file.getOriginalFilename());Path target=root.resolve(key).normalize();if(!target.startsWith(root))throw new ApiException(HttpStatus.BAD_REQUEST,"INVALID_FILE","잘못된 파일입니다.");try(InputStream in=file.getInputStream()){Files.copy(in,target,StandardCopyOption.REPLACE_EXISTING);}Document d=new Document();d.setInheritanceCase(c);d.setOriginalName(safeName(file.getOriginalFilename()));d.setStorageKey(key);d.setMimeType(file.getContentType());d.setSizeBytes(file.getSize());d.setStatus(DocumentStatus.UPLOADED);d.setConsentedAt(Instant.now());return view(documents.save(d));}
    @PostMapping("/api/cases/{caseId}/documents/sample") @ResponseStatus(HttpStatus.CREATED)
    Object sample(@PathVariable Long caseId,Authentication auth)throws IOException{InheritanceCase c=access.owned(caseId,auth);Files.createDirectories(root);String key=UUID.randomUUID()+".pdf";Files.write(root.resolve(key),samplePdf());Document d=new Document();d.setInheritanceCase(c);d.setOriginalName("샘플_안심상속_조회결과.pdf");d.setStorageKey(key);d.setMimeType("application/pdf");d.setSizeBytes(Files.size(root.resolve(key)));d.setStatus(DocumentStatus.UPLOADED);d.setConsentedAt(Instant.now());return view(documents.save(d));}
    ...
    private void validate(MultipartFile f)throws IOException{if(f.isEmpty()||f.getSize()>10*1024*1024)throw new ApiException(HttpStatus.BAD_REQUEST,"INVALID_FILE_SIZE","1바이트 이상 10MB 이하 파일만 업로드할 수 있습니다.");String ext=extension(f.getOriginalFilename());if(!Set.of(".pdf",".png",".jpg",".jpeg").contains(ext)||!MIME.contains(f.getContentType()))throw new ApiException(HttpStatus.UNSUPPORTED_MEDIA_TYPE,"UNSUPPORTED_FILE","PDF, PNG, JPG, JPEG만 지원합니다.");byte[] h=f.getInputStream().readNBytes(8);boolean magic=(ext.equals(".pdf")&&h.length>=4&&h[0]=='%'&&h[1]=='P'&&h[2]=='D'&&h[3]=='F')||(ext.equals(".png")&&h.length>=8&&(h[0]&255)==137&&h[1]=='P'&&h[2]=='N'&&h[3]=='G')||((ext.equals(".jpg")||ext.equals(".jpeg"))&&h.length>=2&&(h[0]&255)==255&&(h[1]&255)==216);if(!magic)throw new ApiException(HttpStatus.UNSUPPORTED_MEDIA_TYPE,"FILE_SIGNATURE_MISMATCH","파일 형식과 내용이 일치하지 않습니다.");}
