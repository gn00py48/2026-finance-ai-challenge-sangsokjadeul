...
public class CaseController {
    private final CaseRepository cases; private final HeirCandidateRepository heirs; private final CaseService access;
    public CaseController(CaseRepository cases,HeirCandidateRepository heirs,CaseService access){this.cases=cases;this.heirs=heirs;this.access=access;}
    ...
    @PatchMapping("/{id}") @Transactional public Object patch(@PathVariable Long id,@Valid @RequestBody CaseInput in,Authentication auth){InheritanceCase c=access.owned(id,auth);boolean impactful=!Objects.equals(c.getAwarenessDate(),in.awarenessDate())||c.isAwarenessDateCertain()!=in.awarenessDateCertain()||!Objects.equals(c.getWillExists(),in.willExists())||!Objects.equals(c.getMinorHeirExists(),in.minorHeirExists());apply(c,in,impactful);heirs.deleteAllByInheritanceCaseId(id);saveHeirs(c,in.heirCandidates());return Map.of("case",view(c),"roadmapImpact",impactful,"impactSummary",impactful?"기산일·유언·미성년 후보 변경은 단계 또는 기한에 영향을 줄 수 있습니다.":"현재 로드맵 순서에 영향이 없는 변경입니다.");}
    private void apply(InheritanceCase c,CaseInput i,boolean dirty){c.setDeceasedDisplayName(i.deceasedDisplayName());c.setDeathDate(i.deathDate());c.setAwarenessDate(i.awarenessDate());c.setAwarenessDateCertain(i.awarenessDateCertain());c.setRelationship(i.relationship());c.setInquiryStatus(i.inquiryStatus());c.setMinorHeirExists(i.minorHeirExists());c.setWillExists(i.willExists());c.setCompletedProcedures(i.completedProcedures());if(dirty)c.setRoadmapDirty(true);}
    private void saveHeirs(InheritanceCase c,List<HeirInput> list){if(list==null)return;for(HeirInput x:list){HeirCandidate h=new HeirCandidate();h.setInheritanceCase(c);h.setDisplayName(x.displayName());h.setRelationship(x.relationship());h.setMinor(x.minor());heirs.save(h);}}
