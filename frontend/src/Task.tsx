...
export default function Task() {
  const { taskId, caseId } = useParams(), qc = useQueryClient();
...
function ResultForm({ step, close, saved, needsReview }: { step: Step; close: () => void; saved: () => Promise<void>; needsReview: boolean }) {
  ...
  return <Sheet title={application ? '신청 결과를 알려주세요' : '확인 결과를 알려주세요'} close={close} busy={busy}>
    <form className="stack" onSubmit={async e => { e.preventDefault(); setBusy(true); setError(''); try { await apiRequest(`tasks/${step.id}/result`, { method: 'PATCH', ...jsonBody({ progressStatus: status, resultDate: date || null, resultText: progressLabels[status], memo }) }); await saved(); } catch (e) { setError((e as Error).message); } finally { setBusy(false); } }}>
