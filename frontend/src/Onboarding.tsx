...
export default function Onboarding() {
  ...
  return <Public>
    ...
    <form className="stack onboarding-form" onSubmit={async e => {
      ...
      {step === 2 && <><Title k="STEP 2 · 관계" t="고인과 어떤 관계인가요?" /><label>사망자와의 관계<select required value={f.relationship} onChange={e => setF({ ...f, relationship: e.target.value })}><option value="">선택해 주세요</option>{['배우자', '자녀', '부모', '형제자매', '기타'].map(x => <option key={x}>{x}</option>)}</select></label><Banner>입력한 관계만으로 법정상속인 여부를 확정하지 않습니다.</Banner></>}
