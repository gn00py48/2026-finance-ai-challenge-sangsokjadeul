...
export default function Finances() {
  ...
  return <>
    ...
    {items.filter(x => matches(x, filter)).map(x => <article className={`card finance-card ${x.itemType === 'REAL_ESTATE' ? 'finance-card--outside' : ''}`} key={x.id}>
      <div className="row"><div className="badges"><span className="status">{x.assetOrDebt === 'ASSET' ? '재산' : '채무'}</span><h2>{x.institution || '기관 미입력'}</h2></div><button className="link" onClick={() => setEditing(x)}>수정</button></div>
